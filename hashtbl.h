/*
    ...
    The HashTable class is a hashtable that takes data stored as a key value
...
HashTable member data
   ...
   - typedef K KeyType: 
	   the key value of the table (fsu::String, etc).
   
   - typedef D DataType: 
	   Is the data stored in table with corresponding key(int,etc).
   
   - typedef fsu::Entry<KeyType,DataType> EntryType: 
		 basic entry type in tables, maps, dictionaries, and associative arrays.
...
1. bool Insert( const KeyType& k, DataType& d)
  ...
  Note: Insert(k,d) must satisfy unimodal semantics. Non-const iterators may be used. 
	...
	Retrieve(i, value) is used to prevent calls to the element operators
    ...
    false and the value is unspecified. 
